  ...
  function build(o, deps, shoruiMei) {
    ...
    var html = DOC.buildDoc({
      ...
      /* ══════════════════════════════════════════════════════════════
         ...
         　区分は日本語で入っています（"定価" "RT" "卸①" …／実データで確認）。
         　そのため「定価」＝一般のお客様の書類に、【法人の三井住友】が出ていました。
         　お客様が違う口座に振り込んでしまいます。実データに「定価」は18件あります。
         　→ 区分の窓口 KAK.isType を通す形に直しました。
         ★英語で直接くらべる形に戻さないでください。また口座がまちがいます。
         　（見張り：tests/test_apps.js の ⑤／tests/test_haribote.js の ④）
         ══════════════════════════════════════════════════════════════ */
