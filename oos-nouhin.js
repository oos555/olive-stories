...
(function (root) {
  ...
  /* ══════════════════════════════════════════════════════════════════
     書類に出す【注文番号】　★2026-09-13 ひろみさん指示で直しました
     ──────────────────────────────────────────────────────────────────
     ひろみさん：「定価購入の人はTKが頭に着くとか、ルール決めたよね？
     　約束事アプリを確認してルールに沿って動くように直して」

     決めごとの親＝【送料・約束ごと Ｓ（soryo.html）の「案件管理番号」】
     　・先頭の記号＝価格区分　TK=定価／OS1=卸①／OS2=卸②／RT／BA=バサラ／
     　　QR=旧ロット／FT=不良品用特価／IT=特別提供価格
     　・そのあと＝受注した日8桁 ＋ ランダム4桁
     　・この番号が付くのは【アプリで受注したもの】と【PDFの見積書・請求書】
     　・RT と バサラ には付かない（※請求書を出すときだけ請求書番号として付く）

     → 書類には【そのままの注文番号】を出します（TK-…／OS1-…／FT-… など）。
     　 RT・RTG・RTGC・BA だけは、頭の記号を外して出します。理由は2つ：
     　 　① 約束ごとで「RT・バサラには案件管理番号が付かない」と決まっている
     　 　② 2026-08-19 ひろみさん「RTという名前が納品書に書かれないように」

     ★2026-08-19 の「どの暗号も外す」は、2026-09-13 に【この形へ差し替え】に
     　なりました（ひろみさん指示）。全部外す形に戻さないでください。
     ★見張り：tests/test_shorui.js の①（本物のこの関数を総当たりで試します）
     ══════════════════════════════════════════════════════════════════ */
  var DOC_NUM_KAKUSU = { RT: 1, RTG: 1, RTGC: 1, BA: 1 };   /* この4つだけ頭を外す */
  function docNumberOf(num) {
    var s = String(num || '').trim();
    var m = s.match(/^([A-Za-z]+[0-9]*)[-－]/);
    if (!m) return s;                                   /* 頭に記号が無ければ、そのまま */
    var kigou = String(m[1]).toUpperCase();
    if (!DOC_NUM_KAKUSU[kigou]) return s;               /* TK・OS1・OS2・FT・IT・QR … そのまま */
    return s.replace(/^[A-Za-z]+[0-9]*[-－]/, '');      /* RT・バサラ … 頭を外す */
  }
  ...
  function build(o, deps, shoruiMei) {
    ...
    if (withAmount) {
      ...
      /* ★2026-09-16 ひろみさん指示：ピッキング手数料の「別途申し受けます」は廃止。
         　えらべるのは【なし／無料サービス／金額】の3つだけです。
         　・'nashi'（なし）… 枠は出しますが「―」と書きます（枠は必ず出す・2026-09-11）
         　・0（無料サービス）… 「無料サービス」
         　・まだ何も押していない … 「―」（勝手に金額を足さない）
         ★送料の「別途申し受けます」は今までどおり残します
         　（2026-09-12 ひろみさん「見積の時に使う可能性大」）。混ぜないでください。 */
      var _whNashi = (String(o.warehouseFee) === 'nashi');
      wakuRows.push({ name: '倉庫ピッキング手数料（バラ出荷）', amount: (_whNashi ? 0 : _whFee),
                      zeroText: ((_whEranda && !_whNashi) ? '無料サービス' : '―') });
    ...
    var html = DOC.buildDoc({
      ...
      /* ══════════════════════════════════════════════════════════════
         ...
         　・合計（税込）と「※印は軽減税率…」の下、【お振込先】・破損の枠の上
         　・最大100文字。書いていなくても【枠はいつも出す】（大きさは決まっている）
         　・見出しはなし。納品書・請求書・領収書（兼も）・RTの納品書、ぜんぶ同じ
         ★枠を「書いたときだけ出す」形に変えないでください（ひろみさん：枠の大きさが決まっていれば混乱しない）
         見張り：tests/test_shorui_kanarazu.js ⑨ */
      okyakuWaku: true,
      okyakuMsg: String(o.okyakuMsg || '').replace(/[\r\n]+/g, ' ').slice(0, 100),
      items: items,
      ...
      /* ══════════════════════════════════════════════════════════════
         ...
         ══════════════════════════════════════════════════════════════ */
      /* ★2026-09-13 ひろみさん指示：振込先は【受注Ａで人が選んだもの】を使います。
         　ひろみさん：「今まで自動だったけど間違ってることがあるから」
         　選ばれていないとき（古い注文など）だけ、今までの自動で決めます。
         ★自動を先に見る形に戻さないでください。 */
      bankIndex: ((o.bankIndex === 0 || o.bankIndex === 1) ? o.bankIndex
                  : ((!KAK.isType(o.customerType, 'general') || o.isCompany) ? 0 : 1)),
      /* ══════════════════════════════════════════════════════════════
