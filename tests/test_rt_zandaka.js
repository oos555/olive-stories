...
/* 「もう書かない」ことを見張る（★2026-09-13 倉庫へのLINEを止めたので足しました） */
function no(label, hay, needle){
  if(String(hay).indexOf(needle) < 0) pass++; else { fail++; fails.push(`${label}  「${needle}」がまだ残っています`); }
}
   ...
   ・タブの説明は文章ではなく図式（★勝手に文章へ戻さない） ── */
{
  ...
  /* ★2026-09-10 ひろみさん指示で名前が変わりました。
     「📦 出荷依頼書に移す」→「📦 発注書へ送る」（もう出荷依頼書は使わないため）。
     ★古い名前に戻さないでください。 */
/* ★2026-09-10 ひろみさん指示で、取り置き・予約は【🗂 取り置き及び発注前予約リスト（ホテル以外）】に
   いったん入り、そのA列を変えたときに発注書へ行き、在庫が減る形になりました。
   図もその形に直したので、見張りもここで新しくします。★古い形に戻さないでください。
   ★2026-09-13 ひろみさん指示：倉庫へのLINEは止めたので、図から「倉庫へLINE」を外しました
   　（倉庫さんは発注書を見て発送されます）。★「倉庫へLINE」に戻さないでください。 */
has('⑧図：RT以外はまずリストに入る', idx, '<b>🗂 リストに1行入る</b>');
has('⑧図：出すのはリストのA列から', idx, '【📦 発注書へ送って出荷を依頼する】');
has('⑧図：そこで在庫が減る（倉庫へLINEは飛ばない）', idx, '<b>自動で 発注書へ ・ 🔵</b>');
no('⑧図：もう画面に「倉庫へLINE」とは出さない', idx, '<b>自動で 発注書・🔵・倉庫へLINE</b>');
