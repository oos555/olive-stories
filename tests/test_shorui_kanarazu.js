...
{
  /* ★「請求書兼領収書」のように【納品書の言葉が無いもの】も入れてあります。
     はじめ『納品書と請求書の両方』で見ていて、ここが抜けました（2026-09-11）。 */
  const shorui = ['納品書兼請求書', '納品書兼領収書', '請求書兼納品書', '請求書兼領収書', '請求書'];
  const aite = [
  ...
  eq('②表に書いたものが、30通りぜんぶで書類に載っている', nuketa.length, 0);
...
/* ══ ⑥ 「納品書」だけの書類には、金額まわりを出さない ══════ */
{
  const h = nouhin(mkOrder({ enclosedDoc:'納品書', customerType:'general' }));
  const rows = gyou(h);
  ok('⑥納品書だけなら送料の行は出さない',       !rows.some(function(r){ return r.indexOf('送料') >= 0; }));
  ok('⑥納品書だけならピッキングの行も出さない', !rows.some(function(r){ return r.indexOf('倉庫ピッキング') >= 0; }));
  inc('⑥金額そのものも出さない', h, 'ご請求金額（税込）', false);
}
