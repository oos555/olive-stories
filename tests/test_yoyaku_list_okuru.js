...
function ylRow(opt){
  ...
  /* ★2026-09-13 在庫の列は3通りになりました：取置分 ／ 予約分 ／ 取置＋予約
     　（ひろみさん指示：取り置きと予約は【商品ごと】に選ぶ／記号は使わない）
     　≪予約分≫が残っている行は、発注書へ送りません（まだ入荷していないため）。 */
  r[2]  = (opt.zaiko === undefined) ? '取置分' : opt.zaiko;
  r[3]  = '2026-11';
  r[4]  = '2026-09-10';
...
/* ══════════════════════════════════════════════════════════════════════
   ①-2 ≪予約分≫が残っている行は、【何回選んでも】送らない（2026-09-13）
   ──────────────────────────────────────────────────────────────────────
   ひろみさん：「予約は、現在ないオイルを予約なので、在庫は減らせない」
   まだ入荷していないものは、どうやっても出せません。
   文字の抜けは「もう一度選べば通す」ですが、これは通しません。
   ★「もう一度選べば通す」形にしないでください。
   ══════════════════════════════════════════════════════════════════════ */
{
  const s = sandbox({ row: { zaiko:'予約分' } });
  s.push();
  eq('①-2 A列は「🗓 まだ出しません」に戻る', s.stateA(), '🗓 まだ出しません');
  inc('①-2 予約分が残っていると言う', s.noteA(), '≪予約分≫が残っている', true);
  inc('①-2 どうすればよいかを書く',   s.noteA(), '一気に在庫へ入れる', true);
  eq('①-2 発注書へは1行も行かない',   s.yuka.rows.length, 0);
  eq('①-2 倉庫LINEは飛ばない',        s.lineSent.length, 0);
  /* もう一度選んでも、やっぱり送らない */
  s.yl.rows[0][0] = '📦 発注書へ送って出荷を依頼する';
  s.push();
  eq('①-2 もう一度選んでも送らない',  s.yuka.rows.length, 0);
  eq('①-2 A列はまた戻る',             s.stateA(), '🗓 まだ出しません');
}
{
  /* 取置分と予約分が混ざった行も、そろうまで送りません（一緒に発送するため） */
  const s = sandbox({ row: { zaiko:'取置＋予約' } });
  s.push();
  eq('①-2 混ざった行も送らない',      s.yuka.rows.length, 0);
  inc('①-2 在庫の中身も見せる',       s.noteA(), '取置＋予約', true);
}
{
  /* ≪取置分≫だけの行は、いままでどおり送れます */
  const s = sandbox({ row: { zaiko:'取置分' } });
  s.push();
  eq('①-2 取置分だけなら送れる',      s.yuka.rows.length, 1);
}
