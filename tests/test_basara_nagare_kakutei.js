...
/* ★2026-09-13 ひろみさん指示：LINEを止めたので、図もその形に直しました。
   ★「💬 本部にLINE 1通」「💬 倉庫にLINE 1通」に戻さないでください。 */
/* ★2026-09-13 ひろみさん「この案内もなくして」＝②とキャンセルの本部LINEの話は図から消しました。
   ★「本部へのLINEは止めました」と書き足すのも無しです（ひろみさんのご指示）。 */
ok('図：☑のときは「発注書を見て気づく」だけ',
  ZU.indexOf('なし ─ 発注書を見て気づく') >= 0);
ok('図：キャンセルは❌の印と赤い取り消し線で分かると書いてある',
  ZU.indexOf('赤い取り消し線</b>で分かります') >= 0);
/* ★2026-09-25 ひろみさん「バサラも同じように、人がチェックを入れた時だけ3分後のLINE。2通目がいかないように」 */
ok('図：🔵のときだけ、3分後に倉庫へ一文と書いてある',
  ZU.indexOf('💬 3分後に倉庫のLINEへ一文だけ') >= 0 && ZU.indexOf('2通目は行きません') >= 0);
/* ★数えるのは【画面に出る札】だけ（コメントの中の注意書きは数えません） */
ok('図：画面に出る「💬 本部にLINE 1通」は請求書の1つだけ',
  (ZU.match(/<p class="msg">💬 本部にLINE 1通<\/p>/g) || []).length === 1,
  '（のこすのは月末の請求書の1本だけです）');
ok('図：本部LINEを止めたという案内も書かない',
  ZU.indexOf('本部へのLINEは止めました') < 0,
  '（2026-09-13 ひろみさん「この案内もなくして」）');
ok('図：もう「💬 倉庫にLINE 1通」と書いていない',
  ZU.indexOf('💬 倉庫にLINE 1通') < 0);
ok('図：倉庫は発注書を見て発送すると書いてある',
  ZU.indexOf('発注書のスプレッドシートを見て発送') >= 0 ||
  ZU.indexOf('発注書</b>のA列が【発送してください】になったら発送') >= 0);
