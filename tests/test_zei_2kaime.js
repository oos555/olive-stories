...
console.log('\n■ Ｄ 倉庫Ｄの納品書に送料・倉庫手数料');
{
  ...
  /* ★2026-09-12 承認済みモック（第7版）：名前は「倉庫ピッキング手数料」 */
  ...
  /* ══════════════════════════════════════════════════════════════════
     ★2026-09-12 ここの決めごとが変わりました。前の2項目は古い見かたでした。
     ──────────────────────────────────────────────────────────────────
     前：「送料を持っていない注文＝0円＝別途申し受けます、合計は商品だけ」
     今：料金を【えらんでいない】注文には、決めごと（oos-shorui-kimari.js）が
     　　お届け先から送料を入れます。だから 0円にも「別途」にもなりません。
     　　「別途申し受けます」は、人が受注Ａで【別途】をえらんだときに出ます
     　　（ひろみさん：「送料は別途申し受けます。も選べるようにして！見積で使う」）。
     ★「送料が無い＝別途」という古い見かたに戻さないでください。
     ══════════════════════════════════════════════════════════════════ */
  /* ══════════════════════════════════════════════════════════════════
     ★2026-09-12（朝）また決めごとが変わりました。ここは2回書き直しています。
     ──────────────────────────────────────────────────────────────────
     ひろみさん：「無料にしてる送料、なんで金額足してるんだよ」
     　　　　　　「無料の意味はわかってる？800円は有料だよ。全然違うんだよ」
     えらんでいない注文に決めごとから880円を入れていたのをやめました。
     人が押していない料金は、1円も足しません。
     ★「決めごとの送料800円が入る」に戻さないでください。 */
  t('Ｄ 料金をえらんでいない注文に、勝手に送料を足さない', h2.indexOf('¥800') >= 0, false);
  t('Ｄ そのとき枠は「別途申し受けます」',                 h2.indexOf('別途申し受けます') >= 0, true);
  /* ★2026-09-11 ひろみさん：「6種類すべての書類に、毎回 倉庫ピッキング手数料と送料の枠は、
     消費税のように当たり前のようにまず枠を設けるようにして」
     → 金額を持っていない注文でも【枠は必ず出す】ようになりました。
     ★「行を出さない」に戻さないでください。 */
  t('Ｄ 金額が無くても送料の枠は出る', h2.indexOf('>送料<') >= 0, true);
  /* 合計＝商品9,500 ＋8%760 ＝ 10,260 円（押していない料金は足さない）
     ★手で計算した数を直に書いています。式に直さないでください
     　（式にすると、親が壊れたときに見張りも同じように壊れて気づけません）。 */
  t('Ｄ そのとき合計は 10,260 円（商品だけ）', h2.indexOf('10,260') >= 0, true);
  t('Ｄ 勝手に足した 11,140 円にはならない',   h2.indexOf('11,140') >= 0, false);

  /* ── 人が【別途申し受けます】をえらんだとき ── こちらが「合計は商品だけ」の場面 */
  const hB = box.buildInvoiceHtml(Object.assign({}, baseOrder, { shippingFee:'betto', warehouseFee:0 }));
  t('Ｄ 別途をえらぶと枠の送料に「別途申し受けます」と出る', hB.indexOf('別途申し受けます') >= 0, true);
  /* 合計＝商品9,500 ＋8%760 ＝ 10,260 円（送料は入らない） */
  t('Ｄ 別途のときの合計は 10,260 円（商品だけ）', hB.indexOf('10,260') >= 0, true);
  t('Ｄ 別途のときは送料800円を足さない', hB.indexOf('11,140') >= 0, false);
  ...
  /* ══════════════════════════════════════════════════════════════════
     ★2026-09-16 この見張りは【張りぼて】でした。
     ──────────────────────────────────────────────────────────────────
     名前は「手数料0円のときは手数料の行を出さない」でしたが、
     2026-09-11にひろみさんが決めた【枠は必ず出す】と真逆のことを見ていました。
     それでも通っていたのは、探していた文字（倉庫ピッキング手数料）が、
     当時の書類の言葉（倉庫ピックアップ料金）と違っていたからです。
     ＝ 何も確かめていませんでした。2026-09-16の名前の統一で、はじめて発覚しました。
     → 正しい決めごとを見るように直しました。★「出さない」に戻さないでください。
     ══════════════════════════════════════════════════════════════════ */
  t('Ｄ 手数料0円でも、手数料の枠は必ず出る', h3.indexOf('倉庫ピッキング手数料') >= 0, true);
  t('Ｄ 手数料0円は「無料サービス」と出る', h3.indexOf('無料サービス') >= 0, true);
}
...
/* ★2026-09-14 業務フロー説明書（gyomu_flow.html）はやめました（ひろみさん指示）。
   ★失った見張り：説明書に「消費税（食品8%／それ以外は全部10%）」と書いてあること／
     説明書が親（oos-zei.js）を指していること。
   　いま税の説明が残っているのは【受注Ａ】と【注文ページ】の2か所です（すぐ上の2行）。
   　説明書を作り直すときは、この2行も戻してください。 */
