...
/* ★2026-09-25 ひろみさん「ロット横の数字は最新になってないから、それなら消して」
   　→ 名簿のロット欄に数（◯本・0 本（入荷前）・残り◯本）は出さなくなりました。この見張りも書き直しました。
   　ロットは在庫0でも出す（上の⑤）ので、「入荷前」と書かなくても、ロットが見えなくなることはありません。 */
ok('⑤名簿のロット欄に在庫の数を出さない（最新ではないため）',
  M.indexOf('0 本（入荷前）') < 0 && M.indexOf("<span class=\"lotqty\">残り '+esc(o.qty)") < 0 && M.indexOf("<span class=\"lotqty now\">'+esc(n.qty)") < 0);
