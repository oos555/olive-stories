...
/* ══════════════════════════════════════════════════════════════════════
   💴 消費税の端数　★2026-09-17 ひろみさん確定（返事が出ました）
   ──────────────────────────────────────────────────────────────────────
   ひろみさん「多くのところで計算方法が四捨五入になっているので、
   　うちだけ切り捨てちゃうと、よそと合わなくなってしまう可能性があるの。
   　なので、**RTだけは切り捨て、他は今までどおり四捨五入**」

   ・RT　　　… 税率ごとの【小計】を出してから【切り捨て】（アイポーターの伝票と合わせる）
   ・それ以外 … 今までどおり【1行ごとに四捨五入】（よその会社と合わせる）

   ★決めごとの置き場所は oos-zei.js だけです（kirisuteKa / zeiGaku）。
   　くわしい見張りは tests/test_zei_hasuu.js（25項目）にあります。
   　ここでは「書類の親が、自分で丸めていないこと」だけを見ます。
   ══════════════════════════════════════════════════════════════════════ */
...
t('⑧ 売上Ｃの請求金額も、同じ親に聞いている',
  fs.readFileSync(R + 'billing.html', 'utf8').indexOf('OOS_ZEI.kirisuteKa(o && o.customerType)') >= 0, true);
